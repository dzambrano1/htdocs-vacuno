...
function insertLombrices($conn, $tagid, $producto, $costo, $fecha) {
    // Prepare the SQL statement
    $query = "INSERT INTO vh_lombrices (vh_lombrices_tagid, vh_lombrices_producto, vh_lombrices_costo, vh_lombrices_fecha) VALUES (?, ?, ?, ?)";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $producto = isset($_POST['vh_lombrices_producto']) ? $_POST['vh_lombrices_producto'] : null;
    $costo = isset($_POST['vh_lombrices_costo']) ? $_POST['vh_lombrices_costo'] : null;
    $fecha = isset($_POST['vh_lombrices_fecha']) ? $_POST['vh_lombrices_fecha'] : null;

    // Call the insert function
    if ($tagid && $producto !== null && $costo !== null && $fecha !== null) {
        insertLombrices($conn, $tagid, $producto, $costo, $fecha);
    } else {
